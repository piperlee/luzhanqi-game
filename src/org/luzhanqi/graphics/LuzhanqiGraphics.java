 ...
 */
public class LuzhanqiGraphics extends Composite implements
    LuzhanqiPresenter.View {
  public interface LuzhanqiGraphicsUiBinder extends
      UiBinder<Widget, LuzhanqiGraphics> {
  }
  ...
  public LuzhanqiGraphics() {
    ...
    LuzhanqiGraphicsUiBinder uiBinder = GWT
        .create(LuzhanqiGraphicsUiBinder.class);
    initWidget(uiBinder.createAndBindUi(this));
    messages = (GameMessages) GWT.create(GameMessages.class);
    gameSounds = GWT.create(GameSounds.class);
    abPanel.setPixelSize(GameSetting.getWidth(), GameSetting.getHeight());
...
//        dragController.registerDropController(dropController);
    ...
    if (Audio.isSupported()) {
      ...
      pieceDown.addSource(gameSounds.pieceDownOgg().getSafeUri().asString(),
    ...
    // add mobile sound
    mobilePieceDown = soundController.createSound(Sound.MIME_TYPE_AUDIO_MPEG,
        "sounds/pieceDown.mp3");
...
//  }
    ...
    for (int i = 0; i < DEPLOY_ROW; i++) {
      for (int j = 0; j < DEPLOY_COL; j++) {
        if (image.equals(deployPanels[i][j].getWidget())) {
          return (turn == Turn.B) ? (i * DEPLOY_COL + j + 25)
              : (i * DEPLOY_COL + j);
        }
      }
    }
  ...
  private boolean enClick(int slotKey, Turn turn) {
    ImmutableList<Integer> set = ImmutableList.<Integer> of(11, 13, 17, 21, 23,
        36, 38, 42, 46, 48);
    if (set.contains(slotKey))
      return false;
    if (turn == Turn.B && slotKey < 30)
      return false;
    if (turn == Turn.W && slotKey > 29)
      return false;
    if (presenter.deployMap.containsValue(slotKey))
      return false;
    return true;
  }
  ...
  private boolean isDeployPhase() {
    return presenter.getState().getIsDeploy();
  }
  ...
  public void setPlayerState(int numberOfOpponentPieces,
    ...
    if (luzhanqiMessage == LuzhanqiMessage.IS_DEPLOY
        || (state.getDW().isPresent() && !state.getDB().isPresent() && presenter
            .isAIGame())) {
          ...
          gamePanels[i][j].addDomHandler(new DoubleClickHandler() {
            ...
            public void onDoubleClick(DoubleClickEvent event) {
              if (!slot.emptySlot()) {
                ...
                doubleClickedImage = (Image) gamePanels[slot.getKey()
                    / GAME_COL][slot.getKey() % GAME_COL].getWidget();
                presenter.pieceDeploy(piece, slot);
                gamePanels[slot.getKey() / GAME_COL][slot.getKey() % GAME_COL]
                    .clear();
              } else {
                if (targetSlot != null && !targetSlot.emptySlot()) {
                  Piece piece = targetSlot.getPiece();
                  // keep image info, use in deployNextPiece
                  doubleClickedImage = (Image) gamePanels[targetSlot.getKey()
                      / GAME_COL][targetSlot.getKey() % GAME_COL].getWidget();
                  presenter.pieceDeploy(piece, targetSlot);
                  gamePanels[targetSlot.getKey() / GAME_COL][targetSlot
                      .getKey() % GAME_COL].clear();
                  targetSlot = null;
                }
              }
            }
  ...
  private void initializeDeployPanels(Turn turn) {
    ...
    for (int i = 0; i < (DEPLOY_ROW * DEPLOY_COL); i++) {
      if (turn == Turn.W)
        ...
        pieceImages.add(PieceImage.Factory.getPieceImage(new Piece(i + 25),
            null));
    }
...
//        }
//        gpDBClick[i][j] = gamePanels[i][j].addDomHandler(
...
//            }, DoubleClickEvent.getType());
  ...
  public void deployNextPiece(Map<Piece, Optional<Slot>> lastDeploy) {
    if (!lastDeploy.isEmpty()) {
      ...
      Slot slot = lastDeploy.get(piece).isPresent() ? lastDeploy.get(piece)
          .get() : null;
      int pieceRow = (piece.getPlayer() == Turn.B) ? piece.getKey()
          / DEPLOY_COL - 5 : piece.getKey() / DEPLOY_COL;
      ...
      if (slot == null) {
        ...
        if (isDrag) {
          ...
          // animation = new PieceMovingAnimation(gameGrid,
          // deployGrid,piece,slot,
          // deployPanels[pieceRow][pieceCol],gamePanels[slotRow][slotCol],pieceDown);
          animation = new PieceMovingAnimation(gameGrid, deployGrid, piece,
              slot, deployPanels[pieceRow][pieceCol],
              gamePanels[slotRow][slotCol], mobilePieceDown);
          animation.run(200);
        }
      }
    }
    ...
    deployBtn.setEnabled(!lastDeploy.isEmpty()
        && presenter.deployMap.size() == 25);
  }

  @Override
  public void nextFromTo(final List<Slot> fromTo) {
    if (!fromTo.isEmpty()) {      
